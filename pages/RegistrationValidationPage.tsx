...
// Define strict interface for form state to ensure all fields are strings
...
const RegistrationValidationPage: React.FC<RegistrationValidationPageProps> = ({ onNavigate }) => {
    ...
    // Initialize state with user data or empty strings to ensure controlled inputs using lazy initialization
    ...
    // Memoize handleChange to prevent unnecessary re-renders of inputs that rely on function equality
        ...
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                     ...
                     {user.accountType === 'corretor' && (
                        <div key="corretor-fields" className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div>
                                <label htmlFor="creciNumber_pf" className="block text-sm font-medium text-gray-700">Número do CRECI (com -F)</label>
                                <input 
                                    type="text" 
                                    name="creciNumber" 
                                    id="creciNumber_pf" 
                             ...
                             <div>
                                <label htmlFor="creciState_pf" className="block text-sm font-medium text-gray-700">Estado do CRECI</label>
                                <select 
                                    name="creciState" 
                                    id="creciState_pf" 
                     ...
                     {user.accountType === 'imobiliaria' && (
                        <div key="imobiliaria-fields" className="space-y-4">
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 <div>
                                    <label htmlFor="creciNumber_pj" className="block text-sm font-medium text-gray-700">CRECI Jurídico (com -J)</label>
                                    <input 
                                        type="text" 
                                        name="creciNumber" 
                                        id="creciNumber_pj" 
                                        value={formData.creciNumber} 
                                        onChange={handleChange} 
                                        placeholder="Ex: 54321-J" 
                                        className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-secondary focus:border-secondary" 
                                        required 
                                    />
                                </div>
                                 <div>
                                    <label htmlFor="creciState_pj" className="block text-sm font-medium text-gray-700">Estado do CRECI</label>
                                    <select 
                                        name="creciState" 
                                        id="creciState_pj" 
                                        value={formData.creciState} 
